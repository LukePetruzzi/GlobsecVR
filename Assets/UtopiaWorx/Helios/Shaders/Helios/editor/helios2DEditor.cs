...
namespace UtopiaWorx.Shaders.PostProcessing
{
	...
	public class Helios2DEditor : Editor 
	{
		...
		static void Command (string input, string RunPath, int CaptureRate, bool DeleteStills, bool Do3D, string SceneName, string AudioPath, bool DebugMode, float Duration,string Scale,string Preset, string FileFormat, string OutputExt, string TemplateName)
		{
			...
			#if UNITY_EDITOR_OSX 
			string args = "-framerate " + CaptureRate.ToString() + " -y -i '" + input + TemplateName + "%05d." + FileFormat + "' " + AudioPath + " -c:v libx264 -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t " + Duration.ToString() + " '" + input + SceneName.Replace(" ","") + ".mp4'";
			if(DebugMode == true)
			{
			...
			#if UNITY_EDITOR_WIN
			string args = "-framerate " + CaptureRate.ToString() + " -y -i \"" + input.Replace(@"/",@"\") + TemplateName + "%05d." + FileFormat + "\" " + AudioPath.Replace(@"/",@"\") + " -c:v libx264 -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t "+ Duration.ToString() + " \""  + input.Replace(@"/",@"\") + SceneName + ".mp4\"";
			if(DebugMode == true)
			{
			UnityEngine.Debug.Log(args);
			}
