...
namespace UtopiaWorx.Shaders.PostProcessing
{
	...
	[CustomEditor(typeof(UtopiaWorx.Helios.Helios))]
	public class HeliosEditor : Editor 
	{
		...
		public override void OnInspectorGUI()
		{
			...
			Helios.Helios MyTarget = (Helios.Helios)target;
			...
			foreach(Component MyCompo in MyComponents)
			{
				if(
					...
					MyCompo.GetType().ToString() == "UtopiaWorx.Helios.Helios" ||
				...
				{
					bool Shown = EditorPrefs.GetBool("Helios3DComponentWarning");
					if(Shown == false)
					{
						EditorUtility.DisplayDialog("Add Component Warning", "Hey there Cowboy, \r\nIt looks like you are trying to add something to the Helios camera rig, and thats totally cool...\r\nHowever\r\nIf you're looking to add post processsing effects to Helios, you will need to add them to the child object of Helios, called \"Stitcher\" to make them work in your saved images. We won't bug you with this warning again.","Got It!");
						EditorPrefs.SetBool("Helios3DComponentWarning" ,true);
					}
			...
			if(Logo == null)
			{
				Logo = Resources.Load("Images/Helios") as Texture2D;
			}
			...
			string[] ProjectionModes = {"360 Video","Little Planet", "360 Stereoscopic"};
			string[] QualitySettings = {"Preview (480)","Mobile (720)","Desktop (1080)", "DVD (4K)", "VR (8K)","YouTube Voodoo"};
			...
			string[] OutputFormats = {"MPEG4","GIF"};
			...
			SerializedProperty sp_EdgeStitching = serializedObject.FindProperty ("EdgeStitching");
			...
			SerializedProperty sp_Process3D = serializedObject.FindProperty ("Process3D");
			...
			SerializedProperty sp_FlipWindows= serializedObject.FindProperty ("FlipWindows");
			SerializedProperty sp_GammaCorrect= serializedObject.FindProperty ("GammaCorrect");
			SerializedProperty sp_SetreoZoom= serializedObject.FindProperty ("SetreoZoom");
			SerializedProperty sp_ProjectionMode= serializedObject.FindProperty ("ProjectionMode");
			...
			SerializedProperty sp_SyncRotation = serializedObject.FindProperty ("SyncRotation");
			SerializedProperty sp_StereoOffset = serializedObject.FindProperty ("StereoOffset");
			...
			switch(UtopiaWorx.Helios.Helios.HeliosStatus)
			{
			case 0:
			...
			if(sp_FoldoutGeneral.boolValue)
			{
				...
				sp_ProjectionMode.intValue =  EditorGUILayout.Popup("Projection Mode",sp_ProjectionMode.intValue,ProjectionModes);

				if(sp_ProjectionMode.intValue == 1)
				{
					sp_SetreoZoom.floatValue = EditorGUILayout.Slider(new GUIContent("Zoom","How far to zoom in"),sp_SetreoZoom.floatValue,0.0f,1.0f);
				}
				if(sp_ProjectionMode.intValue == 2)
				{
					sp_StereoOffset.floatValue = EditorGUILayout.Slider(new GUIContent("IPD Offset","How far to offset the stereo view"), sp_StereoOffset.floatValue,0.01f,2.0f);
...
//				}
				...
				{
					sp_Quality.intValue =  EditorGUILayout.Popup("Quality",sp_Quality.intValue,QualitySettings);
					if(sp_Quality.intValue >2 && SystemInfo.graphicsMemorySize < 1025)
					{
				...
				EditorGUILayout.BeginHorizontal();
				GUILayout.FlexibleSpace();
				sp_SyncRotation.boolValue =  EditorGUILayout.Toggle("Sync Camera Rotation",sp_SyncRotation.boolValue);
				EditorGUILayout.EndHorizontal();
				...
				sp_EdgeStitching.intValue = EditorGUILayout.IntSlider(new GUIContent("Edge Smoothing","If your edges seem rough, you can smooth the stitching with this option"),sp_EdgeStitching.intValue,0,5);
			...
			if(sp_FoldoutAdvanced.boolValue)
			{
				...
				sp_GammaCorrect.boolValue = EditorGUILayout.Toggle("Gamma Boost",sp_GammaCorrect.boolValue);
			...
			if(sp_FoldoutOutput.boolValue)
			{
				...
				sp_ClipDuration.floatValue = EditorGUILayout.Slider(new GUIContent("Clip Length (Seconds)","How many frames to actually encode in the video."),sp_ClipDuration.floatValue,0.001f,99999.999f);
				...
				EditorGUILayout.BeginHorizontal();
				GUILayout.FlexibleSpace();
				sp_Process3D.boolValue = EditorGUILayout.Toggle("3D Video Metadata",sp_Process3D.boolValue);
				EditorGUILayout.EndHorizontal();
				...
				EditorGUILayout.EndHorizontal();
					...
					EditorGUILayout.BeginHorizontal();
					GUILayout.FlexibleSpace();
					sp_FlipWindows.boolValue = EditorGUILayout.Toggle("Flip Image",sp_FlipWindows.boolValue);
					EditorGUILayout.EndHorizontal();
				#else
				sp_FlipWindows.boolValue = false;
				#endif
			...
			{
				if(System.IO.Directory.Exists(Application.dataPath + "/" + "UtopiaWorx/Helios/Bin") == true)
				{
				if(GUILayout.Button(MakeVideo, GUILayout.Width(64), GUILayout.Height(64)))
				{
					...
					bool Do3D = sp_Process3D.boolValue;
					...
					if(System.IO.File.Exists(path + SceneName + ".mp4") == true)
					{
						System.IO.File.Delete(path + SceneName + ".mp4");
					}
					string FileFormat = "";
					if(sp_JPEG.boolValue == true)
					{
					...
					var thread = new Thread(delegate () {Command(path,PathDir, FRames,Delete,Do3D, SceneName,AudioPath,DoDebug,LocalDur,Scale,Preset,FileFormat, OutPutExt, TheFileFormat);});
					thread.Start();
				}
				}
				else
		...
		static void Command (string input, string RunPath, int CaptureRate, bool DeleteStills, bool Do3D, string SceneName, string AudioPath, bool DebugMode, float Duration, string Scale, string Preset, string FileFormat, string OutputExt, string TemplateName)
		{
			System.IO.File.Delete(input + TemplateName + "0000." + FileFormat);
			#if UNITY_EDITOR_OSX 
			string args = "";

			args = "-framerate " + CaptureRate.ToString() + " -y -i '" + input + TemplateName + "%05d." + FileFormat + "' " + AudioPath + " -c:v libx264 -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t " + Duration.ToString() + " '" + input + SceneName.Replace(" ","") + "." + "mp4" +"'";


			//string args = "-framerate 9 -i '" + input + "%05d." + FileFormat +   "' -vf scale=531x299,transpose=1,crop=299,431,0,100 " + " '" + input + SceneName.Replace(" ","") + "." +OutputExt +"'";
			if(DebugMode == true)
			{
			...
			UtopiaWorx.Helios.Helios.HeliosStatus = 1;
			var process = Process.Start(processInfo);
			process.WaitForExit();
			process.Close();
			UtopiaWorx.Helios.Helios.HeliosStatus = 0;
...
//					./ffmpeg  -i '/HeliosTemp/3D_Demo.mp4' -i '/HeliosTemp/palette.png' -filter_complex "fps=60,scale=854:-1:flags=lanczos[x];[x][1:v]paletteuse"  -r 20 '/HeliosTemp/3D_Demo.gif'
			...
			if(Do3D)
			{
				var processInfo2 = new 	ProcessStartInfo(RunPath + "Spatial Media Metadata Injector.app/Contents/MacOS/Spatial Media Metadata Injector");
				processInfo2.CreateNoWindow = false;
				UtopiaWorx.Helios.Helios.HeliosStatus = 2;
			...
			#if UNITY_EDITOR_WIN
			string args = "-framerate " + CaptureRate.ToString() + " -y -i \"" + input.Replace(@"/",@"\") + TemplateName + "%05d." + FileFormat + "\" " + AudioPath.Replace(@"/",@"\") + " -c:v libx264 -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t "+ Duration.ToString() + " \""  + input.Replace(@"/",@"\") + SceneName.Replace(" ","") + ".mp4\"";
			if(DebugMode == true)
			{
				UnityEngine.Debug.Log(args);
			}
			...
			if(Do3D)
			{
			var processInfo2 = new 	ProcessStartInfo(RunPath + "Spatial Media Metadata Injector.exe");
			processInfo2.CreateNoWindow = false;
			UtopiaWorx.Helios.Helios.HeliosStatus = 2;
			var process2 = Process.Start(processInfo2);
			process2.WaitForExit();
